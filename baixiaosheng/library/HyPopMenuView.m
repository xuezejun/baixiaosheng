...
-(void)CirculatingItmes
{
    ...
    UIScrollView *scrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 100, kW, kH-180)];
    [self addSubview:scrollView];
    NSInteger index = 0;
    for (MenuLabel *Obj in _ItmesArr) {
        CGFloat buttonX,buttonY;
        buttonX = (index % HowMucHline) * ButtonWidth;
        buttonY = ((index / HowMucHline) * (ButtonHigh +10)) +0;
        ...
        if(buttonY+ButtonHigh>=(kH-180))
        {
            scrollView.frame=CGRectMake(0, 100, kW, kH-180);
        }else
        {
            scrollView.frame=CGRectMake(0, 100, kW, buttonY+ButtonHigh);
        }
        scrollView.contentSize=CGSizeMake(0,buttonY+ButtonHigh);
...
//    UIImage *image = [UIImage imageNamed:MenuData.iconName];
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:MenuData.iconName] placeholderImage:[UIImage imageNamed:IMAGEBACK] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
