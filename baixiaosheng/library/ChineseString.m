...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        chineseString.string=[NSString stringWithString:[stringArr objectAtIndex:i][@"name"]];
       ...
       // NSLog(@"string====%@",chineseString.string);
        ...
        //判断首字符是否为字母
        NSLog(@"首字母%@",chineseString.string);
        NSString *regex = @"[A-Za-z]+";
        NSPredicate*predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
        if ([predicate evaluateWithObject:[chineseString.string substringToIndex:[chineseString.string length]>0?1:0]])
        {
            //首字母大写
