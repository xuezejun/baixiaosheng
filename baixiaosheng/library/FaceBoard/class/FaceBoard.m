...
#define FACE_COUNT_CLU  [[NSString stringWithFormat:@"%f",WIDTH(WINDOW)/44] intValue]
...
- (id)init {
//    [[NSString stringWithFormat:@"%f",WIDTH(WINDOW)/44] intValue];
    self = [super initWithFrame:CGRectMake(0, 0,WIDTH(WINDOW), 216)];
    if (self) {
        ...
        //表情盘
        faceView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, WIDTH(WINDOW), 190)];
        faceView.pagingEnabled = YES;
        faceView.contentSize = CGSizeMake((FACE_COUNT_ALL / FACE_COUNT_PAGE + 1) * WIDTH(WINDOW), 190);
        ...
        for (int i = 1; i <= FACE_COUNT_ALL; i++) {
            ...
            //计算每一个表情按钮的坐标和在哪一屏
            CGFloat x = (((i - 1) % FACE_COUNT_PAGE) % FACE_COUNT_CLU) * FACE_ICON_SIZE + (WIDTH(WINDOW)-FACE_COUNT_CLU*44)/2 + ((i - 1) / FACE_COUNT_PAGE * WIDTH(WINDOW));
        ...
        back.frame = CGRectMake(WIDTH(WINDOW)-(320-272), 182, 38, 28);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    [facePageControl setCurrentPage:faceView.contentOffset.x / WIDTH(WINDOW)];
    [facePageControl updateCurrentPageDisplay];
}

- (void)pageChange:(id)sender {

    [faceView setContentOffset:CGPointMake(facePageControl.currentPage * WIDTH(WINDOW), 0) animated:YES];
